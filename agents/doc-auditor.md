...
2. **Template shape:** each area file has the expected sections from `_template.md` (Propósito, Âmbito, Regras, Referências, Termos, Gaps) — allow empty tables, not missing structure.
3. **No fake completeness:** status Canónico without PM-backed content is a finding; prefer Rascunho + `INFORMAÇÃO AUSENTE`.
